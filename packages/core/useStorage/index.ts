...
export interface StorageOptions<T> extends ConfigurableEventFilter, ConfigurableWindow, ConfigurableFlush {
  ...
  /**
   * Time to wait (in ms) before resetting sync read/write detection
   *
   * @default 150
   */
  syncResetTimeout?: number
}
    ...
    watchWithFilter(
      ...
      () => {
        try {
          ...
          synced = true
          syncReset = setTimeout(() => {synced = false}, syncResetTimeout)
        }
        catch (e) {
          onError(e)
        }
