...
describe.skipIf(isBelowNode18)('useFetch', () => {
  ...
  it('should be refresh correctly', async () => {
    vi.useFakeTimers()
    const { execute } = useFetch('https://example.com', { immediate: false, refresh: 100 })
    await execute()
  ...
  it('should cache correctly', async () => {
    vi.useFakeTimers()
    const { execute, clearCache } = useFetch('https://example.com', { immediate: false, refresh: 100, cache: true })
    await execute()
    await vi.advanceTimersByTimeAsync(1000)
    expect(fetchSpy).toBeCalledTimes(1)
    clearCache()
  ...
  it('should cacheExpiration correctly', async () => {
    vi.useFakeTimers()
    const { execute, clearCache } = useFetch('https://example.com', { immediate: false, refresh: 100, cache: true, cacheExpiration: 600 })
    await execute()
    await vi.advanceTimersByTimeAsync(1000)
  ...
  it('should clearCache correctly', async () => {
    vi.useFakeTimers()
    const { execute, clearCache } = useFetch('https://example.com', { immediate: false, cache: true })
    await execute()
    clearCache()
    await execute()
    expect(fetchSpy).toBeCalledTimes(2)
  ...
  it('should cacheResolver correctly', async () => {
    ...
    const { execute } = useFetch('https://example.com', { immediate: false, cache: true, cacheResolve }).post({ a: 1, b: 2 })
    const { execute: execute2 } = useFetch('https://example.com', { immediate: false, cache: true, cacheResolve }).post({ a: 1, c: 2 })
    await execute()
    await execute2()
    expect(fetchSpy).toBeCalledTimes(1)
  })
