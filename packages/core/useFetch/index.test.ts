...
describe('useFetch', () => {
  ...
  test('should create an instance of useFetch with a base url', async() => {
    ...
    expect(fetchMock.mock.calls[0][1]!.headers).toMatchObject(new Headers({ 'Authorization': 'test', 'Accept-Language': 'en-US' }))
  ...
  test('should run the beforeFetch function and add headers to the request', async() => {
    ...
    expect(fetchMock.mock.calls[0][1]!.headers).toMatchObject(new Headers({ 'Authorization': 'my-auth-token', 'Accept-Language': 'en-US' }))
  })
