...
If back-end data will update in a short interval, setting `refresh` option can help refresh data auto

```ts
const url = ref('https://my-api.com/user/1')

// data will refresh every 60000 ms
const { cancelRefresh, data } = useFetch(url, { refresh: 60000 })

cancelRefresh() // cancel data refresh
```
...
Setting the `cache` option to true will cache fetch response data, the cache key is generated by the `cacheResolve` function. cache expiration can be set with `cacheExpiration` option, cache data will store by WeakRef so don't worry about memory leaks. when browser gc trigger, not quoted cache data will be cleared. also a `clearCache` function was offer to clear all cache by manually

```ts
const url = ref('https://my-api.com/user/1')

const { clearCache, data } = useFetch(url, {
  cache: true,
  cacheExpiration: 10 * 60 * 1000, // default infinity
  cacheResolve: ({ url, playload, type }) => url + type + JSON.stringify(playload), // cache key resolve, default ({url}) => url
})
  .post(playload)
